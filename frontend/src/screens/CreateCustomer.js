...
const CreateCustomer = ({ history }) => {
	...
	const onAddCustomer = async (e) => {
		e.preventDefault()

		console.log(alerts)
		await axios.post(`/api/customers/`, formData).catch(function (err) {
	...
	return (
		<div>
			...
			<span className='title-2'>Add Customer</span>
			{alerts &&
				alerts.length > 0 &&
				alerts.map((alert) => (
			...
			<div className='card-details'>
				<form onSubmit={(e) => onAddCustomer(e)}>
					...
					<div className='details-container'>
						<Button className='add-btn' type='submit' onClick={onAddCustomer}>
							Add Customer
						</Button>
					</div>
				</form>
